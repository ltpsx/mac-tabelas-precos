...
def main():
    ...
    tarefas = [
        ...
        {
            "comando": 'cd "C:\\Users\\compr\\.claude-worktrees\\precos_project\\bold-turing" && git add app/ ata/ birigui/ prudente/ index.html',
            "descricao": "3/4 - Adicionar arquivos ao Git"
        },

        # ETAPA 4: GIT COMMIT + PUSH (no worktree)
        {
            "comando": f'cd "C:\\Users\\compr\\.claude-worktrees\\precos_project\\bold-turing" && git commit -m "Atualizacao automatica - {datetime.now().strftime("%d/%m/%Y %H:%M")}" && git push origin bold-turing:main',
